...
void GenerateItems(void)
{
    ...
    for (i = 0; i < random; i++)
    {
        ...
        if (random16 == ITEM_EXP_SHARE || random16 == ITEM_SOOTHE_BELL || random16 == ITEM_AMULET_COIN || random16 == ITEM_EVERSTONE || random16 == ITEM_LUCKY_EGG || random16 == ITEM_SMOKE_BALL)
        {
            i--;
            continue;
        }
